 ...
 */
export declare const PROTOCOLIZATION_EXECUTION_EVENT_TYPES: Readonly<{
    readonly executed: "ProtocolizationExecuted";
}>;
export type ProtocolizationExecutionEventType = (typeof PROTOCOLIZATION_EXECUTION_EVENT_TYPES)[keyof typeof PROTOCOLIZATION_EXECUTION_EVENT_TYPES];
/**
 * "The Asset Protocolization workflow executed over this exact case revision and
 * produced this result."
 *
 * Every field is a copy of the corresponding field of the result it announces,
 * and `occurredAt` is the result's own `executedAt` — the event does not read a
 * clock of its own, so it can never claim a different instant from the artifact
 * it describes.
 */
export interface ProtocolizationExecutedEvent {
    readonly eventType: typeof PROTOCOLIZATION_EXECUTION_EVENT_TYPES.executed;
    ...
    /** The result's `executedAt`, verbatim. */
    readonly occurredAt: UtcDateTime;
    readonly correlationId?: CanonicalId;
}
export type ProtocolizationExecutionEvent = ProtocolizationExecutedEvent;
