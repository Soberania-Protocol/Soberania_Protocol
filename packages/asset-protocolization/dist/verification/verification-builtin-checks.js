...
const verification_ports_1 = require("./verification-ports");
...
const OUTCOME_PRECEDENCE = Object.freeze({
    [verification_outcome_1.VerificationCheckOutcome.Fail]: 4,
    [verification_outcome_1.VerificationCheckOutcome.Unavailable]: 3,
    [verification_outcome_1.VerificationCheckOutcome.Warning]: 2,
    [verification_outcome_1.VerificationCheckOutcome.ManualReview]: 1,
    [verification_outcome_1.VerificationCheckOutcome.Pass]: 0,
});
/**
 ...
 */
function strongest(observations, fallback) {
    let held;
...
function requirementRefs(requirementIds) {
    return requirementIds.map((requirementId) => ({
        kind: verification_result_1.VerificationInputKind.Requirement,
        ref: requirementId,
    }));
}
/** Every material of one kind that names this requirement among its correlations. */
function materialsFor(materials, requirementId, kind) {
    return materials.filter((material) => material.kind === kind && material.requirementIds.includes(requirementId));
}
/**
 ...
 * participates.
 */
const COUNTED_MATERIAL_KIND = Object.freeze({
    [requirements_1.AssetRequirementKind.Declaration]: case_material_1.ProtocolizationMaterialKind.Declaration,
    [requirements_1.AssetRequirementKind.Evidence]: case_material_1.ProtocolizationMaterialKind.Evidence,
    [requirements_1.AssetRequirementKind.Attestation]: case_material_1.ProtocolizationMaterialKind.Attestation,
});
// ---------------------------------------------------------------------------
 ...
 * a possible one.
 */
exports.requiredMaterialPresentCheck = Object.freeze({
    checkId: 'check.material.present',
    execute(context) {
        const considered = context.requirementProgress.filter((progress) => progress.kind !== requirements_1.AssetRequirementKind.Verification);
        const missing = considered
            .filter((progress) => progress.obligation === requirements_1.AssetRequirementObligation.Required &&
            progress.materialStatus === case_state_1.ProtocolizationRequirementMaterialStatus.Pending)
            .map((progress) => progress.requirementId);
        if (missing.length > 0) {
            return {
                outcome: verification_outcome_1.VerificationCheckOutcome.Fail,
        ...
        const unresolved = considered
            .filter((progress) => progress.conditionStatus === case_state_1.ProtocolizationRequirementConditionStatus.Unresolved &&
            progress.materialStatus === case_state_1.ProtocolizationRequirementMaterialStatus.Pending)
            .map((progress) => progress.requirementId);
        if (unresolved.length > 0) {
            return {
                outcome: verification_outcome_1.VerificationCheckOutcome.ManualReview,
        ...
        return {
            outcome: verification_outcome_1.VerificationCheckOutcome.Pass,
 ...
 */
exports.minimumMaterialCountCheck = Object.freeze({
    checkId: 'check.material.minimum-count',
    execute(context) {
        const constrained = context.assetProfile.requirements.filter((requirement) => requirement.obligation === requirements_1.AssetRequirementObligation.Required &&
        ...
        const short = constrained.filter((requirement) => {
            const kind = COUNTED_MATERIAL_KIND[requirement.kind];
...
function evidencedStrategies(context, requirement) {
    ...
    if (context.subject.contentIdentity !== undefined ||
        materialsFor(context.materials, requirement.id, case_material_1.ProtocolizationMaterialKind.ContentIdentity).length > 0) {
        evidenced.add(requirements_1.AssetIdentityStrategy.ContentIdentity);
    }
    if (context.subject.subjectRef.externalReference !== undefined ||
        materialsFor(context.materials, requirement.id, case_material_1.ProtocolizationMaterialKind.ExternalReference).length > 0) {
        evidenced.add(requirements_1.AssetIdentityStrategy.ExternalReference);
    }
    if (materialsFor(context.materials, requirement.id, case_material_1.ProtocolizationMaterialKind.RegistryEntry).length > 0) {
        evidenced.add(requirements_1.AssetIdentityStrategy.RegistryEntry);
    }
    return evidenced;
}
/**
 ...
 */
exports.identityStrategyCheck = Object.freeze({
    checkId: 'check.identity.strategy',
    execute(context) {
        const requirements = context.assetProfile.requirements.filter((requirement) => requirement.kind === requirements_1.AssetRequirementKind.Identity &&
            requirement.obligation === requirements_1.AssetRequirementObligation.Required);
        ...
        for (const requirement of requirements) {
            ...
            if (requirement.registry !== undefined) {
                ...
                const nonConforming = materialsFor(context.materials, requirement.id, case_material_1.ProtocolizationMaterialKind.RegistryEntry).filter((material) => material.kind === case_material_1.ProtocolizationMaterialKind.RegistryEntry &&
                    ...
                    observations.push({
                        ...
                        inputRefs: [
                            ...
                            ...nonConforming.map((material) => ({
                                kind: verification_result_1.VerificationInputKind.Material,
...
// ---------------------------------------------------------------------------
function ageSeconds(observedAt, now) {
 ...
 * observation arrives.
 */
exports.evidenceFreshnessCheck = Object.freeze({
    checkId: 'check.evidence.freshness',
    execute(context) {
        const requirements = context.assetProfile.requirements.filter((requirement) => requirement.kind === requirements_1.AssetRequirementKind.Evidence && requirement.freshness !== undefined);
        if (requirements.length === 0) {
            return {
                outcome: verification_outcome_1.VerificationCheckOutcome.Pass,
        ...
        let evaluated = 0;
        for (const requirement of requirements) {
            const constraint = requirement.freshness;
            const materials = materialsFor(context.materials, requirement.id, case_material_1.ProtocolizationMaterialKind.Evidence);
            for (const material of materials) {
                if (material.kind !== case_material_1.ProtocolizationMaterialKind.Evidence)
                    continue;
                const refs = [
                    ...requirementRefs([requirement.id]),
                    { kind: verification_result_1.VerificationInputKind.EvidenceRecord, ref: material.evidenceRef },
                ];
                const receipt = context.evidenceReceipts.find((candidate) => candidate.materialId === material.materialId ||
                    candidate.evidenceRef === material.evidenceRef);
                const observedAt = receipt?.observedAt;
                if (observedAt === undefined) {
                    observations.push({
                        outcome: verification_outcome_1.VerificationCheckOutcome.Unavailable,
                ...
                const basis = receipt === undefined
                    ? refs
                    : [...refs, { kind: verification_result_1.VerificationInputKind.IntakeReceipt, ref: receipt.intakeId }];
                ...
                if (age < 0) {
                    observations.push({
                        outcome: verification_outcome_1.VerificationCheckOutcome.Warning,
                ...
                if (constraint.observedAfter !== undefined &&
 ...
 */
exports.declarationClaimTypeCheck = Object.freeze({
    ...
    async execute(context) {
        ...
        for (const record of context.declarations) {
            const refs = [
                { kind: verification_result_1.VerificationInputKind.DeclarationRecord, ref: record.declarationId },
                { kind: verification_result_1.VerificationInputKind.ClaimRecord, ref: record.claimRef },
            ];
            const resolution = await resolver.resolveClaim(record.claimRef);
            if (resolution.status === verification_ports_1.VerificationResolutionStatus.Unavailable) {
                observations.push({
                    outcome: verification_outcome_1.VerificationCheckOutcome.Unavailable,
                    reasonCode: 'dependency.unavailable',
            ...
            if (resolution.status === verification_ports_1.VerificationResolutionStatus.NotFound || resolution.claim === undefined) {
                observations.push({
                    outcome: verification_outcome_1.VerificationCheckOutcome.Unavailable,
        ...
        return strongest(observations, {
            ...
            inputRefs: context.declarations.map((record) => ({
                kind: verification_result_1.VerificationInputKind.DeclarationRecord,
...
// ---------------------------------------------------------------------------
function declarantKey(declarant) {
 ...
 */
exports.competingDeclarationCheck = Object.freeze({
    checkId: 'check.declaration.competing',
    execute(context) {
        const declarationRequirementIds = new Set(context.assetProfile.requirements
            .filter((requirement) => requirement.kind === requirements_1.AssetRequirementKind.Declaration)
            .map((requirement) => requirement.id));
        const declarantsByRequirement = new Map();
        for (const record of context.declarations) {
            for (const requirementId of record.requirementIds) {
                if (!declarationRequirementIds.has(requirementId))
                    continue;
                const held = declarantsByRequirement.get(requirementId) ?? new Set();
 ...
 * why — never `Fail`.
 */
exports.contentDigestCheck = Object.freeze({
    checkId: 'check.content.digest',
    async execute(context) {
        const refs = [
            { kind: verification_result_1.VerificationInputKind.Subject, ref: context.subject.subjectRef.sovereignAssetId },
        ];
        ...
        if (resolution.status === verification_ports_1.VerificationResolutionStatus.NotFound || resolution.bytes === undefined) {
            return {
                outcome: verification_outcome_1.VerificationCheckOutcome.Unavailable,
        ...
        const verified = (0, identity_1.verifyContentIdentity)(resolution.bytes, expected);
        if (verified.valid) {
            return {
                outcome: verification_outcome_1.VerificationCheckOutcome.Pass,
 ...
 */
exports.BUILT_IN_VERIFICATION_CHECKS = Object.freeze([
    exports.requiredMaterialPresentCheck,
    exports.minimumMaterialCountCheck,
    exports.identityStrategyCheck,
    exports.evidenceFreshnessCheck,
    exports.declarationClaimTypeCheck,
    exports.competingDeclarationCheck,
    exports.contentDigestCheck,
]);
