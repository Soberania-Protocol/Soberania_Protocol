...
exports.evaluateIdentityRequirement = evaluateIdentityRequirement;
exports.evaluateDeclarationRequirement = evaluateDeclarationRequirement;
exports.evaluateEvidenceRequirement = evaluateEvidenceRequirement;
exports.evaluateVerificationRequirement = evaluateVerificationRequirement;
exports.evaluateAttestationRequirement = evaluateAttestationRequirement;
exports.evaluateRequirement = evaluateRequirement;
exports.isBlockingObligation = isBlockingObligation;
const requirements_1 = require("../requirements");
const case_material_1 = require("../case/case-material");
const attestation_preparation_1 = require("../attestation/attestation-preparation");
const review_actions_1 = require("../attestation/review-actions");
const verification_builtin_checks_1 = require("../verification/verification-builtin-checks");
...
function reason(reasonCode, requirementId, extra = {}) {
 ...
 * compatibility.
 */
function materialsFor(protocolizationCase, requirementId, kind) {
    return protocolizationCase.materials.filter((material) => material.kind === kind && material.requirementIds.includes(requirementId));
}
const materialIdsOf = (materials) => materials.map((material) => material.materialId);
/** The profile's declared minimum, or the structural minimum of one. */
const minimumOf = (declared) => typeof declared === 'number' && Number.isInteger(declared) && declared > 0 ? declared : 1;
/**
 ...
 */
function unevaluatedConstraint(requirementId, constraint, extra = {}) {
    return reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.constraintUnevaluated, requirementId, {
        constraint,
        ...extra,
    });
}
 ...
 */
function evidenceCredentialConforms(evidence, constraint) {
    ...
    if (acceptedStatuses === undefined)
        return true;
    return typeMatches.some((ref) => ref.status !== undefined && acceptedStatuses.includes(ref.status.status));
}
/** Whether any registry entry the evidence record names conforms to the constraint. */
function evidenceRegistryConforms(evidence, constraint) {
    return (evidence.registryRefs ?? []).some((entryRef) => (0, verification_builtin_checks_1.registryEntryConforms)(entryRef, constraint));
}
// ---------------------------------------------------------------------------
// Identity
// ---------------------------------------------------------------------------
 ...
 */
function evaluateIdentityRequirement(context, requirement) {
    const subject = context.protocolizationCase.subject;
    const byStrategy = {
        [requirements_1.AssetIdentityStrategy.ContentIdentity]: case_material_1.ProtocolizationMaterialKind.ContentIdentity,
        [requirements_1.AssetIdentityStrategy.ExternalReference]: case_material_1.ProtocolizationMaterialKind.ExternalReference,
        [requirements_1.AssetIdentityStrategy.RegistryEntry]: case_material_1.ProtocolizationMaterialKind.RegistryEntry,
    };
    const materials = [];
    ...
    for (const strategy of requirement.acceptedStrategies) {
        ...
        const conforming = strategy === requirements_1.AssetIdentityStrategy.RegistryEntry && requirement.registry !== undefined
            ? found.filter((material) => material.kind === case_material_1.ProtocolizationMaterialKind.RegistryEntry &&
                (0, verification_builtin_checks_1.registryEntryConforms)(material.registryEntryRef, requirement.registry))
            : found;
        if (conforming.length < found.length) {
            nonConforming.push(...found
        ...
        if (strategy === requirements_1.AssetIdentityStrategy.ContentIdentity && subject.contentIdentity !== undefined) {
            evidenced.add(strategy);
        }
        if (strategy === requirements_1.AssetIdentityStrategy.ExternalReference &&
    ...
    if (nonConforming.length > 0) {
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.materialIncompatible, requirement.id, {
            constraint: 'registry',
    ...
    const required = requirement.satisfaction === requirements_1.AssetRequirementSatisfaction.All
        ? requirement.acceptedStrategies.length
        : 1;
    if (evidenced.size === 0) {
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.materialMissing, requirement.id, {
            required,
            observed: 0,
        }));
    }
    else if (evidenced.size < required) {
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.materialInsufficient, requirement.id, {
            required,
 ...
 */
function evaluateDeclarationRequirement(context, requirement) {
    const materials = materialsFor(context.protocolizationCase, requirement.id, case_material_1.ProtocolizationMaterialKind.Declaration);
    const minimum = minimumOf(requirement.minimumCount);
    const recordsByClaim = new Map();
    for (const record of context.declarations) {
        if (record.requirementIds.includes(requirement.id))
            recordsByClaim.set(record.claimRef, record);
    }
    const qualifying = new Set();
    const incompatible = new Set();
    for (const material of materials) {
        if (material.kind !== case_material_1.ProtocolizationMaterialKind.Declaration)
            continue;
        const record = recordsByClaim.get(material.claimRef);
        if (record !== undefined) {
            const typeMatches = record.claimType === requirement.claimType;
            const subtypeMatches = requirement.claimSubtype === undefined || record.claimSubtype === requirement.claimSubtype;
 ...
 */
function evaluateEvidenceRequirement(context, requirement) {
    const materials = materialsFor(context.protocolizationCase, requirement.id, case_material_1.ProtocolizationMaterialKind.Evidence);
    const minimum = minimumOf(requirement.minimumCount);
    const present = new Set();
    for (const material of materials) {
        if (material.kind === case_material_1.ProtocolizationMaterialKind.Evidence)
            present.add(material.evidenceRef);
    }
    const blockers = [];
    const warnings = [];
    if (requirement.freshness !== undefined) {
    ...
    const qualifying = new Set();
    const unestablished = new Map();
    const incompatible = new Map();
    const note = (held, constraint, ref) => {
    ...
    for (const evidenceRef of present) {
        ...
        if (record === undefined) {
            ...
            if (requirement.registry !== undefined)
                note(unestablished, 'registry', evidenceRef);
            if (requirement.credential !== undefined)
                note(unestablished, 'credential', evidenceRef);
            continue;
        }
        let conforms = true;
 ...
 */
function standingForOutcome(reportAgainst, result, extra = {}) {
    const executionIds = [result.executionId];
    const at = (code) => reason(code, reportAgainst, { checkId: result.checkId, executionIds, ...extra });
    switch (result.outcome) {
        case verification_outcome_1.VerificationCheckOutcome.Pass:
            return { satisfied: true, executionIds };
        case verification_outcome_1.VerificationCheckOutcome.Warning:
            return {
                satisfied: true,
                warning: at(readiness_reason_1.PROTOCOLIZATION_READINESS_WARNING_CODES.verificationWarning),
                executionIds,
            };
        case verification_outcome_1.VerificationCheckOutcome.Fail:
            return {
                satisfied: false,
                blocker: at(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.verificationFail),
                executionIds,
            };
        case verification_outcome_1.VerificationCheckOutcome.ManualReview:
            return {
                satisfied: false,
                blocker: at(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.verificationManualReview),
                executionIds,
            };
        case verification_outcome_1.VerificationCheckOutcome.Unavailable:
            return {
                satisfied: false,
                blocker: at(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.verificationUnavailable),
                executionIds,
            };
        default:
            return {
                satisfied: false,
                blocker: at(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.verificationMissing),
                executionIds,
            };
    }
}
 ...
 */
function resolveCheckStanding(context, declaringRequirementId, checkId, reportAgainst, extra = {}) {
    const forPair = context.verificationResults.filter((result) => result.requirementId === declaringRequirementId && result.checkId === checkId);
    const current = (0, verification_projections_1.listLatestVerificationResults)(forPair.filter((result) => (0, verification_projections_1.isVerificationResultCurrentForRevision)(result, context.evaluatedCaseRevision)))[0];
    if (current !== undefined)
        return standingForOutcome(reportAgainst, current, extra);
    if (forPair.length > 0) {
        const executionIds = forPair.map((result) => result.executionId);
        return {
            satisfied: false,
            blocker: reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.verificationStale, reportAgainst, {
                checkId,
                executionIds,
    ...
    return {
        satisfied: false,
        blocker: reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.verificationMissing, reportAgainst, {
            checkId,
            ...extra,
        }),
        executionIds: [],
 ...
 */
const EVIDENCE_FRESHNESS_CHECK_ID = verification_builtin_checks_1.evidenceFreshnessCheck.checkId;
/**
 ...
 */
function dischargeEvidenceFreshness(context, requirement) {
    const declaring = context.profile.requirements.filter((candidate) => candidate.kind === requirements_1.AssetRequirementKind.Verification &&
    ...
    const standings = declaring.map((declarer) => resolveCheckStanding(context, declarer.id, EVIDENCE_FRESHNESS_CHECK_ID, requirement.id, {
    ...
    if (discharged !== undefined) {
        return {
            ...
            warnings: [
                ...
                reason(readiness_reason_1.PROTOCOLIZATION_READINESS_WARNING_CODES.constraintDelegated, requirement.id, {
                    constraint: 'freshness',
                    checkId: EVIDENCE_FRESHNESS_CHECK_ID,
                    executionIds: discharged.executionIds,
                }),
                ...standings.flatMap((standing) => standing.warning === undefined ? [] : [standing.warning]),
            ],
        };
    }
    return {
        blockers: standings.flatMap((standing) => standing.blocker === undefined ? [] : [standing.blocker]),
        warnings: [],
    };
}
/**
 ...
 */
function evaluateVerificationRequirement(context, requirement) {
    const standings = requirement.checkIds.map((checkId) => resolveCheckStanding(context, requirement.id, checkId, requirement.id));
    ...
    const satisfied = requirement.satisfaction === requirements_1.AssetRequirementSatisfaction.All
        ? satisfiedCount === requirement.checkIds.length
        : satisfiedCount > 0;
    const warnings = standings.flatMap((standing) => standing.warning === undefined ? [] : [standing.warning]);
    ...
    const freshnessBlockers = requirement.freshness === undefined
        ? []
    ...
    const unsatisfiedReasons = standings.flatMap((standing) => standing.satisfied || standing.blocker === undefined ? [] : [standing.blocker]);
    return {
        materialIds: [],
 ...
 */
function qualifyingAttestation(context, requirement, material) {
    if (material.kind !== case_material_1.ProtocolizationMaterialKind.Attestation) {
        return { ok: false, reasonCode: readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.attestationDecisionMissing };
    }
    const decision = context.reviewDecisions.find((candidate) => candidate.action === review_actions_1.ProfessionalReviewAction.Attest &&
    ...
    if (decision.scope === undefined ||
        ...
        return {
            ok: false,
            reasonCode: readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.attestationTypeIncompatible,
        };
    }
    ...
    if (attestation === undefined ||
        ...
        proofRefs.length === 0 ||
        !proofRefs.every((proofRef) => (0, attestation_preparation_1.isUsableProofRef)(proofRef))) {
        return {
            ok: false,
            reasonCode: readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.attestationUnprovenArtifact,
        };
    }
 ...
 * unsatisfiable.
 */
function coversCurrentRevision(context, requirement, coveredThrough) {
    for (let revision = coveredThrough + 1; revision <= context.evaluatedCaseRevision; revision += 1) {
        const accountedFor = context.reviewDecisions.some((decision) => decision.action === review_actions_1.ProfessionalReviewAction.Attest &&
 ...
 */
function currentDecisions(context, requirement) {
    const forRequirement = context.reviewDecisions.filter((decision) => decision.attestationRequirementId === requirement.id);
    if (forRequirement.length === 0)
        return [];
    const latestBasis = forRequirement.reduce((highest, decision) => Math.max(highest, decision.reviewBasisRevision), Number.NEGATIVE_INFINITY);
 ...
 * ```
 */
function evaluateAttestationRequirement(context, requirement) {
    const materials = materialsFor(context.protocolizationCase, requirement.id, case_material_1.ProtocolizationMaterialKind.Attestation);
    const minimum = minimumOf(requirement.minimumCount);
    const blockers = [];
    const warnings = [];
    ...
    const qualifyingDecisionIds = new Set();
    const staleRefs = [];
    const staleDecisionIds = [];
    let staleCoveredThrough;
    ...
    for (const material of materials) {
        ...
        if (outcome.ok) {
            ...
            const coveredThrough = outcome.decision.resultingCaseRevision ?? outcome.decision.reviewBasisRevision;
    ...
    if (currentRefs.size < minimum && staleRefs.length > 0) {
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.attestationCurrencyUnresolved, requirement.id, {
    ...
    if (currentActions.size > 1) {
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.reviewConflicting, requirement.id, {
            decisionIds: current.map((decision) => decision.decisionId),
            atCaseRevision: current[0]?.reviewBasisRevision,
        }));
    }
    else if (currentActions.has(review_actions_1.ProfessionalReviewAction.Reject)) {
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.reviewRejected, requirement.id, {
            decisionIds: current.map((decision) => decision.decisionId),
        }));
    }
    else if (currentActions.has(review_actions_1.ProfessionalReviewAction.RequestMoreEvidence)) {
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.reviewMoreEvidenceRequired, requirement.id, {
            decisionIds: current.map((decision) => decision.decisionId),
        }));
    }
    else if (currentActions.has(review_actions_1.ProfessionalReviewAction.Abstain)) {
        ...
        blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.reviewPending, requirement.id, {
            decisionIds: current.map((decision) => decision.decisionId),
        }));
    }
    if (currentRefs.size < minimum) {
        if (materials.length === 0) {
            blockers.push(reason(readiness_reason_1.PROTOCOLIZATION_READINESS_BLOCKER_CODES.attestationMissing, requirement.id, {
                required: minimum,
                observed: 0,
            }));
        }
        ...
        const undecided = context.reviewRequests.filter((request) => request.attestationRequirementId === requirement.id &&
            !context.reviewDecisions.some((decision) => decision.reviewRequestId === request.reviewRequestId));
 ...
 */
function evaluateRequirement(context, requirement) {
    switch (requirement.kind) {
        ...
        default:
            throw new readiness_errors_1.ProtocolizationReadinessError(readiness_errors_1.PROTOCOLIZATION_READINESS_ERROR_CODES.unsupportedRequirementKind, `The pinned profile declares a requirement kind this evaluator does not handle`, {
                reasonCodes: [readiness_errors_1.PROTOCOLIZATION_READINESS_ERROR_CODES.unsupportedRequirementKind],
...
function isBlockingObligation(obligation) {
    return (obligation === requirements_1.AssetRequirementObligation.Required ||
        obligation === requirements_1.AssetRequirementObligation.Conditional);
}
