...
import { PROTOCOLIZATION_EXECUTION_EVENT_TYPES } from './execution-events';
...
import {
  ...
  validateProtocolizationExecutionRequest,
  validateProtocolizationResult,
} from './execution-validation';
import {
  PROTOCOLIZATION_EXECUTION_RESULT_SCHEMA_VERSION,
  protocolizationMaterialRecordRef,
} from './protocolization-result';
import type {
  ProtocolizationExecutionMaterialRef,
  ProtocolizationReadinessBasis,
 ...
 */
function collectRefs<T extends string>(
  evaluation: ProtocolizationReadinessEvaluation,
  select: (assessment: ProtocolizationReadinessEvaluation['requirementAssessments'][number]) =>
    | readonly T[]
    | undefined,
): readonly T[] {
...
function quoteReadiness(
  ...
  return {
    ...
    state: ProtocolizationReadinessState.Ready,
    ...
    verificationExecutionIds: collectRefs<VerificationExecutionId>(
      evaluation,
      (assessment) => assessment.verificationExecutionIds,
    ),
    reviewDecisionIds: collectRefs<ProfessionalReviewDecisionId>(
      evaluation,
      (assessment) => assessment.reviewDecisionIds,
    ),
    attestationRefs: collectRefs<CanonicalAttestationId>(
      evaluation,
      (assessment) => assessment.attestationRefs,
    ),
  };
}
 ...
 */
function quoteMaterial(
  protocolizationCase: ProtocolizationCase,
): readonly ProtocolizationExecutionMaterialRef[] {
  return protocolizationCase.materials.map((material) => {
    const ref = protocolizationMaterialRecordRef(material);
    return {
 ...
 */
export function executeProtocolization(
  context: ProtocolizationCaseContext,
  protocolizationCase: ProtocolizationCase,
  readinessEvaluation: ProtocolizationReadinessEvaluation,
  request: ProtocolizationExecutionRequest,
): ProtocolizationExecutionOutcome {
  assertExecutingTenantOwnsCase(context, protocolizationCase);
  const profile = resolveExecutionProfile(context, protocolizationCase);
  assertCaseIsExecutable(protocolizationCase, profile);

  const requestValidation = validateProtocolizationExecutionRequest(request);
  if (!requestValidation.admitted) {
    failExecution(
      requestValidation.reasons.includes(
        PROTOCOLIZATION_EXECUTION_VALIDATION_CODES.invalidResultId,
      )
        ? PROTOCOLIZATION_EXECUTION_ERROR_CODES.resultIdInvalid
        : PROTOCOLIZATION_EXECUTION_ERROR_CODES.requestInvalid,
      `Refusing an inadmissible protocolization execution request: ${requestValidation.reasons.join(', ')}`,
      {
  ...
  const evaluation = assertReadinessAuthorizesExecution(protocolizationCase, readinessEvaluation);
  assertExpectedRevision(protocolizationCase, evaluation, request.expectedCaseRevision);

  const executedAt = readExecutionInstant(context, protocolizationCase);

  const result: ProtocolizationResult = {
    schemaVersion: PROTOCOLIZATION_EXECUTION_RESULT_SCHEMA_VERSION,
  ...
  const resultValidation = validateProtocolizationResult(result);
  if (!resultValidation.admitted) {
    failExecution(
      PROTOCOLIZATION_EXECUTION_ERROR_CODES.resultInvalid,
      `Refusing to return an invalid ProtocolizationResult: ${resultValidation.reasons.join(', ')}`,
      {
  ...
  const event: ProtocolizationExecutedEvent = {
    eventType: PROTOCOLIZATION_EXECUTION_EVENT_TYPES.executed,
  ...
  return deepFreeze({ result, event });
}
 ...
 */
export function reconstituteProtocolizationResult(value: unknown): ProtocolizationResult {
  const validation = validateProtocolizationResult(value);
  if (!validation.admitted) {
    failExecution(
      PROTOCOLIZATION_EXECUTION_ERROR_CODES.resultInvalid,
      `Refusing to reconstitute an invalid ProtocolizationResult: ${validation.reasons.join(', ')}`,
      { reasonCodes: validation.reasons },
    );
  }
  return deepFreeze(value as ProtocolizationResult);
}
